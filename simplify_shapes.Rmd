...
noise_per_lvl <- mutate(noise, level = cut(L_N, breaks = seq(50, 80, by = 10), right = FALSE, ordered_result = TRUE)) %>%
  group_by(level) %>%
...
All in all, it's wise to first think about simplifying your spatial data before you continue to work with the it. Spatial datasets are often very large and depending on your use-case you may not need a high level of detail. It may even be absolutely necessary to reduce the complexity of your data, e.g. when the computational capacities are limited or when you produce interactive maps for the web. As shown, R has all the tools that you need to simplify spatial data in the packages sf and rmapshaper.
